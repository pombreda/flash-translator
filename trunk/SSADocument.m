...
static ssastyleline SSA_DefaultStyle = {
	@"Default",@"Helvetica",
	20,
...
-(NSArray *)makeSubLines:(NSMutableArray*)linesa
{
	int num = [linesa count], i;
	NSMutableArray *outa = [[NSMutableArray alloc] init];
...
-(void) loadFile:(NSString*)path width:(float)width height:(float)height
{
	NSString *ssa = [[NSString stringFromUTF8File:path] stringByStandardizingNewlines];
	if (!ssa) return;
	...
	pair = [nextLine pairSeparatedByString:@": "];
	formatstring = (version == S_ASS) ?
	...
	_lines = [self makeSubLines:doclines];
}
